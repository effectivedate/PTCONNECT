								...
								<div class="inner_contents"><!-- 코치정보(패딩탑) -->
									<div><!-- 코치정보 -->
										<div>
											...
											<c:if test="${not empty trainerSalesList}">
											
											<c:forEach var="po" items="${trainerSalesList}" varStatus="loop">
									        <span id="mbNo_${loop.index}" style="display: none;">${po.mbNo}</span>
									        <div class="orderListBox">
									            ...
									            <div class="orderContainer">
									                ...
									                <span class="orderText">구매자 이름 :</span>
									                <span class="orderText">구매자 전화번호 :</span>
									            </div>
									            <div class="orderContainer">
									                <span id="odNo_${loop.index}">${po.odNo}</span>
									                <span class="orderText">${po.tnName}</span>
									                <span class="orderText">
									                	<c:if test="${not empty po.mbName}">${po.mbName}</c:if>
									                	<c:if test="${empty po.mbName}">${po.nmName}</c:if>
									                </span>
									                <span class="orderText">
									                	<c:if test="${not empty po.oderMbPhone}">${po.oderMbPhone}</c:if>
									                	<c:if test="${empty po.oderMbPhone}">${po.nmPhone}</c:if>
									                </span>
...
<%-- 									                <span class="orderName">${po.odCoupon}</span> --%>
											...
											<c:if test="${empty trainerSalesList}">
												<div style="padding: 150px 0;">
		                            				<h2 style="font-weight: bold; text-align: center;">판매내역이 없습니다!</h2>
		                            			</div>
											</c:if>
	                            				...
	                            				<ul class="pagination">
	                            					<li class="paginate_button page-item previous">
		                            					<c:if test="${empty startDate  && empty endDate}"> 
		                            						...
		                            						<a href="trainerSales${pm.startPage-1}" aria-controls="dataTable" class="page-link">
		                            							◀
		                            						</a>
		                            						</c:if>
		                            					</c:if>
		                            					<c:if test="${not empty startDate && not empty endDate}">
		                            						<c:if test="${pm.prev eq true}">
		                            						<a href="trainerSales${pm.startPage-1}?startDate=${startDate}&endDate=${endDate}" aria-controls="dataTable" class="page-link">
		                            							◀
		                            						</a>
		                            						</c:if>
		                            					</c:if>
		                            						...
		                            						<c:choose>
		                            							<c:when test="${i eq pm.currentPage}">
		                            								<li class="paginate_button page-item active">
					                            						<a href="trainerSales${i}" aria-controls="dataTable" class="page-link">
					                            							${i}
					                            						</a>
					                            					</li>
		                            							</c:when>
		                            				...
		                            				<c:if test="${not empty startDate && not empty endDate}">
		                            					<c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}" step="1">
										                    <c:choose>
										                        <c:when test="${i eq pm.currentPage}">
										                            <li class="paginate_button page-item active">
										                                <a href="trainerSales${i}?startDate=${startDate}&endDate=${endDate}" aria-controls="dataTable" class="page-link">
										                                    ${i}
										                                </a>
										                            </li>
										                        </c:when>
	                            					...
	                            					<li class="paginate_button page-item next">
	                            						<c:if test="${empty startDate && empty endDate}"> 
		                            						...
		                            						<a href="trainerSales${pm.endPage+1}" aria-controls="dataTable" class="page-link">
		                            							▶
		                            						</a>
		                            						</c:if>
		                            					</c:if>
		                            					<c:if test="${not empty startDate && not empty endDate}">
		                            						<c:if test="${pm.next eq true && pm.endPage > 0}">
		                            						<a href="trainerSales${pm.endPage+1}?startDate=${startDate}&endDate=${endDate}" aria-controls="dataTable" class="page-link">
		                            							▶
		                            						</a>
		                            						</c:if>
		                            					</c:if>
...
function paymentCancle(index) {
	console.log('index :', index);
            ...
            if (confirmation) {
                ...
                var mbNo = document.getElementById('mbNo_' + index).textContent;
...
<script>
	...
	function searchByDateRange() {
	    ...
	    $.ajax({
	        url: '${pageContext.request.contextPath}/trainerSales1?startDate=' + startDate + '&endDate=' + endDate, 
	        type: 'POST',
	        contentType: 'application/json',
	        data: JSON.stringify(result),
	        success: function () {
		        	location.href='${pageContext.request.contextPath}/trainerSales1?startDate=' + startDate + '&endDate=' + endDate;
	        },
	        error: function (xhr, status, error) {
	            console.log(xhr.responseText);
	            console.log(status);
	...
	function resetDateRange() {
        ...
        location.href="${pageContext.request.contextPath}/trainerSales1";
    }
